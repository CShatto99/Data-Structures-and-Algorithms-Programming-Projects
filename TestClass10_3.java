...
// This project implements a method that takes an array of key values from a 2-3-4 tree and sorts them
// SOLUTION AT BOTTOM OF TREE234 CLASS
public class TestClass10_3 {
	...
	public static class Tree234 {
		...
		// PROJECT SOLUTION (selection sort)
		public void sort(int dataItems[], int size) {
			int in, out;
			for(out = 1; out < size; out++) {
				int temp = dataItems[out];
				in = out;
				while(in > 0 && dataItems[in-1] >= temp) {
					dataItems[in] = dataItems[in-1];
					--in;
				}
				dataItems[in] = temp;
			}
